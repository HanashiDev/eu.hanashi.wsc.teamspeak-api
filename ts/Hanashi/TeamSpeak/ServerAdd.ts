...
export class ServerAdd {
    constructor() {
        document.getElementById('queryType')?.addEventListener('change', (ev) => this.changeQueryType(ev));
    }

    protected changeQueryType(e: Event) {
        const target: HTMLSelectElement | null = e.target as HTMLSelectElement | null;
        if (target == null) {
            return;
        }
        ...
        if (target.value == 'http' || target.value == 'https') {
            ...
            serverPortDescriptionText = 'wcf.page.teamspeakAdd.virtualServerID.description';
        ...
        if (virtualServerPortContainer != null) {
            const virtualServerPortLabel = virtualServerPortContainer.querySelector('label[for="virtualServerPort"]') as HTMLLabelElement | null;
        ...
        if (passwordContainer != null) {
            const passwordLabel = passwordContainer.querySelector('label[for="password"]') as HTMLLabelElement | null;
            if (passwordLabel != null) {
                passwordLabel.innerHTML = Language.get(passwordText);
            }
        }
        
        const virtualServerPort = document.getElementById('virtualServerPort') as HTMLInputElement | null;
        if (virtualServerPort != null) {
            virtualServerPort.value = serverPort;
        }

        const queryPortField: HTMLInputElement | null = document.getElementById('queryPort') as HTMLInputElement | null;
